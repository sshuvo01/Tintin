...
class Vector
{
  public:
   ...
   Vector(double v) { xyz[0] = xyz[1] = xyz[2] = v; } // sss
...
//  Comparisons
   ...
   static const lux::Vector CompWiseMult(const Vector& v1, const Vector& v2) 
   {
	   return lux::Vector(v1.X() * v2.X(), v1.Y() * v2.Y(), v1.Z() * v2.Z());
   }
   // Outer product
   /*
   const Matrix operator&&(const Vector& v) const
   {
	   Matrix mat = Matrix();
	   for (int i = 0; i < 3; i++)
	   {
